...
function categoryUrl(value: string) { return value === "All News" ? "/" : `/category/${value.toLowerCase().replace(/&/g, "and").replace(/\s+/g, "-")}`; }
...
export function NewsTerminal({ initialStories, initialCursor, initialCategory = "All News", initialSector }: { initialStories: Story[]; initialCursor: string | null; initialCategory?: string; initialSector?: string }) {
  ...
  return <div className="news-shell">
    <header className="topbar">
      ...
      <div className="top-actions"><label className="search"><Icon name="search" size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search companies, markets or topics" aria-label="Search news"/></label><a className="terminal-cta-top" href="https://quantifyterminal.com/#download" target="_blank" rel="noopener noreferrer">Download Terminal <Icon name="arrow" size={14}/></a></div>
    </header>
